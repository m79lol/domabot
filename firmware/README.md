...
- выставляет статус `STS::EMERGENCY` в регистре статуса `REG_INP::STS`;
...
| `EMERGENCY`   | `6`  | Авария. Получен аварийный сигнал. |
...
| `MOVE`   | `2`  | При остановке в `MODE::TRG` | Движение в заданную позицию |
| `UPDATE` | `3`  | При остановке                      | Применить настройки из Holding регистров с адресами 04-13 |
| `SAVE`   | `4`  | При остановке                      | Тоже самое, что и `UPDATE`, при этом настройки записываются в EEPROM |
| `MODE`   | `5`  | Всегда                             | Переключение режима работы (предварительно вызывает `CMD::BRAKE`). |
| `DIR`    | `6`  | В `MODE::DRCT`                     | Смена направления движения указанного в `REG_HLD::DIR` |
