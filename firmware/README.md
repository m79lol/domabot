...
1. к позиции по команде Modbus, режим `MODE_TRG`;
2. в направлении по команде Modbus, режим `MODE_DRCT`;
3. с пульта, режим `MODE_WRD`.

Получив команду на перемещение к позиции (режим `MODE_TRG`), контроллер переводит расстояние в миллиметрах в угол поворота колеса, затем в угол вала двигателя и количество необходимых шагов. Движение осуществляется в пределах заданных ускорения (торможения) и максимальной скорости.

При команде на перемещение в направлении, контроллер разгоняет двигатели до максимальной скорости в заданных пределах ускорения (торможения), и удерживает скорость пока не будет дана иная команда (для режима `MODE_DRCT`) или иной сигнал (для режима `MODE_WRD`).
...
Ручной режим управления с проводного пульта (`MODE_WRD`) включается и отключается аппаратно - переключателем.
...
- выставляет статус `STS_EMRGENCY` в регистре статуса `REG_INP_STS`;
- игнорирует поступающие команды в командном регистре `REG_HLD_CMD`;
- продолжает обновлять регистры данных.

При пропадании аварийного сигнала в регистр статуса записывается значение `STS_OK`, обработка команд возобновляется, активный режим при этом не меняется.
...
- `WRD_MODE_SWITCH_PIN` - номер пина переключения режима управления с пульта (`MODE_WRD`);
...
| `REG_INP_STPR_L` |  `02`  | Статус *левого* мотора |
| `REG_INP_POS_L`  |  `03`  | Текущая позиция *левого* мотора, в мм |
| `REG_INP_STPR_R` |  `04`  | Статус *правого* мотора |
| `REG_INP_POS_R`  |  `05`  | Текущая позиция *правого* мотора, в мм |
...
| `STS_ERR_DIR`     | `5`  | Ошибочное направление движения в режиме `MODE_DRCT` (значение регистра `REG_HLD_MAN_DIR` ) |
| `STS_EMRGENCY`    | `6`  | Авария. Получен аварийный сигнал. |
| `STS_ERR_UNKNOWN` | `99` | Неизвестная ошибка |

Статус мотора `REG_INP_STPR`_X принимает следующие значения:
- 0 - стоим;
- 1 - едем;
- 4 - тормозим.
...
| `REG_HLD_MODE`   |  `14`  | Режим работы |
| `REG_HLD_DIR`    |  `15`  | Направление движения |
...
| Имя         | Код  | Описание                 |
| ----------- | ---- | ------------------------ |
| `MODE_TRG`  |   0  | Движение к целевой позиции (режим по умолчанию) |
| `MODE_DRCT` |   1  | Движение в целевом направлении |
| `MODE_WRD`  |   2  | Движение в целевом направлении с аппартного пульта (включается только аппаратно) |

Направление движения в направлении через регистр `REG_HLD_MAN_DIR` может быть следующим:
| Имя            | Код  | Описание                 |
| -------------- | ---- | ------------------------ |
| `DIR_STOP`     |   0  | Cтоп (по умолчанию, при включении режима направления) |
| `DIR_FORWARD`  |   1  | Прямолинейное движение в перед |
| `DIR_RIGHT`    |   2  | Поворот на месте на право (по часовой стрелке) |
| `DIR_BACKWARD` |   3  | Прямолинейное движение назад |
| `DIR_LEFT`     |   4  | Поворот на месте на лево (против часовой стрелки) |
